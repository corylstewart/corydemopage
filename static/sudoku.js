    ...
    for (var i = 0; i < 9; i++){
        for (var j = 0; j < 9; j++){
            ...
            if (!this_cell.readOnly) {
                this_cell.value = ''
                this_cell.className = 'sudoku-cell empty-cell'
            }
        }
    }
}
...
function show_errors() {
    ...
    for (var i = 0; i < 9; i++) {
        for (var j = 0; j < 9; j++) {
            ...
            var loc = (i * 9) + j
            var this_cell = document.getElementById(cell_name)
            if (this_cell.className != 'sudoku-cell full-cell' && this_cell.value != '') {
                if ( this_cell.value == puzzle_solution.substring(loc,loc+1)) {
                    this_cell.className = 'sudoku-cell empty-cell'
                }
                else {
                    this_cell.className = 'sudoku-cell wrong-cell'
                }
            }
        }
    }
...
function show_solution() {
    for (var i = 0; i < 9; i++) {
        for (var j = 0; j < 9; j++) {
            ...
            var loc = (i * 9) + j
            var this_cell = document.getElementById(cell_name)
            if (this_cell.className != 'sudoku-cell full-cell') {
                this_cell.className = 'sudoku-cell empty-cell'
